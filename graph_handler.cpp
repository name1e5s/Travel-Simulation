...
GraphHandler::GraphHandler(QObject *parent) : QObject(parent), m_cityGraph() {
  ...
  QObject::connect(&m_simulateTimer, &QTimer::timeout, this, &GraphHandler::printNewLog);
}
...
void GraphHandler::generatePlanResult() {
    ...
    logFile.clear();
    logFile.push_back("================================");
    logFile.push_back(QDateTime::currentDateTime().toString("     yyyy-MM-dd hh:mm:ss.zzz    "));
    logFile.push_back("(C) 2019 Travel-Simulation Group");
    logFile.push_back("================================");
    logFile.push_back("用户输入");
    logFile.push_back(QString("起始城市: %1").arg(QString::fromStdString(m_cityGraph.index_city[m_traveller.source_city_index])));
    logFile.push_back(QString("目标城市: %1").arg(QString::fromStdString(m_cityGraph.index_city[m_traveller.dest_city_index])));
    logFile.push_back(QString("策略类型: %1").arg(m_traveller.plan_type));
    logFile.push_back("必经城市:");
    for(auto x : m_traveller.middle_city_index) {
            logFile.push_back(QString("    %1").arg(QString::fromStdString(m_cityGraph.index_city[x])));
    }
    logFile.push_back("================================");
    ...
    logFile.push_back(QString("路线数目: %1").arg(m_traveller.plan_result.size()));
    logFile.push_back("================================");
    ...
    t.beginHour = m_planResult.startHour + (tmpHour >= 0? tmpHour : 24 + tmpHour);
    t.resultType = ROUTE;
    ...
    for(size_t i = 1; i < m_traveller.plan_result.size(); i++) {
        if((m_planResult.result.back().beginHour +
            m_traveller.plan_result[i - 1].duration) % 24 != m_traveller.plan_result[i].start_time) {
            // Wait here.
        ...
        if(m_planResult.result.back().resultType == CITY)
            ...
            tmpHour = (m_traveller.plan_result[i].start_time) % 24 -
                    (m_planResult.result.back().beginHour +
                     m_planResult.result.back().transport.duration) % 24;
...
void GraphHandler::printNewLog() {
    ...
    QString routeString = realLogDate.toString("yyyy-MM-dd") + " " +
                            QString::number(currentTime % 24) + "时 "
            + QString::fromStdString(m_planResult.result[log_i].transport.tran_name) + " "
            + "次" + getTranName(m_planResult.result[log_i].transport.tran_type) + " 从 %1 到 %2";
    ...
    if(currentTime == m_planResult.startHour + m_totalTime) {
        logUpdated("于 " + realLogDate.toString("yyyy-MM-dd") + " " +
                   QString::number(currentTime % 24) + "时到达终点 %1",m_planResult.result.back().transport.dest_city,0,1);
        m_simulateTimer.stop();
        emit simulationDone();
        return;
    }
    ...
    currentTime++;
    if(m_planResult.result.size() != log_i + 1 && currentTime == m_planResult.result[log_i + 1].beginHour) {
        log_i++;
    }
}
...
void GraphHandler::receiveNewLog(QString string) {
    logFile.push_back(string);
}
